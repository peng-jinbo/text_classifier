...
def nb_predict(sentence):
    ...
    # prob = softmax_res(aff_name2log_p.values())
...
if __name__ == "__main__":
    ...
    with open(test_path, "r") as f:
        for line in f.readlines():
            ...
            if not line:
                continue
            surface, _, label = line.split("\t\t")
            ori_l.append(surface)
            label_l.append(int(label))
    ...
    for i in range(61):
        ...
        start = i * (len(ori_l) // 60)
        end = start + len(ori_l) // 60 if i != 60 else len(ori_l)
        with Pool() as pool:
            for p in tqdm(pool.imap(nb_predict, ori_l[start: end]), total=end - start - 1):
                prob_l.append(p)

    with open("./prob.pkl", "wb") as f:
        pickle.dump(prob_l, f)
    ...
    print("evaluating...")
    Pl, Rl = [], []
    for T in tqdm(range(0, 10001)):
        thres = T / 10000
        tmp_res = [int(i <= thres) for i in prob_l]
        prec = precision_score(label_l, tmp_res, zero_division=0)
        Pl.append(prec)
        reca = recall_score(label_l, tmp_res)
        Rl.append(reca)
    
    res = np.zeros((10001, 2))
    res[:, 0] = Pl
    res[:, 1] = Rl
